...
// undefined value means: nothing's selected
    ...
    if (isAstObject(astObject)) {
        ...
        switch (astObject.concept) {
            ...
            case "Data Attribute": return <div
                className={asClassNameArgument("attribute", (selection.selected === astObject) && "selected")}
                onClick={action((_) => {
                    selection.selected = astObject
                    // Note: if value is already observable, it doesn't get wrapped again despite selection being deeply-observable.
                    // (Effectively, 'selected' has now been decorated with 'obersvable.ref'.)
                })}>
                <span className="keyword ws-right">the</span>
