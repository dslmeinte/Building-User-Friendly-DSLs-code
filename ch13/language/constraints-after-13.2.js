    ...
    switch (astObject.concept) {
        case "Attribute": {
            ...
            if (isAstObject(settings["value"])) {
                ...
                if (typeOfAttribute !== typeOfValue) {
...
const typeIssuesForBinaryOperator = (binaryOperation, ancestors) => {
    ...
    switch (settings["operator"]) {
        ...
        case "of": {
            if (leftType !== "percentage") {
                issues.push(`The left operand of this 'of' operator has type '${leftType}', but must have type 'percentage'`)
            }
            if (leftType !== "amount") {
                issues.push(`The right operand of this 'of' operator has type '${rightType}', but must have type 'amount'`)
            }
            break
        }
        default: {
